...
function PricingCard({ plan, onCTAClick }: PricingCardProps) {
  ...
  return (
    <div className={`pricing-card-wrapper ${colorClass} group relative w-full h-[520px] rounded-[20px] flex items-center justify-center transition-all duration-[600ms] ease-out md:hover:scale-105 md:hover:-translate-y-[10px] p-[3px]`} style={{ perspective: '1000px' }}>
      <div 
      ...
      >
        {/* Front Side */}
        <div 
          className={`pricing-card-inner absolute inset-0 bg-bg-elev rounded-[17px] overflow-hidden transition-shadow duration-500 ease-out ${
            isMobile 
        ...
        >
          ...
          <button
            ...
            className="absolute top-4 right-4 z-20 w-8 h-8 rounded-full bg-accent/90 hover:bg-accent flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-lg"
        ...
        >
          ...
          <div className="relative z-10 p-6 h-full flex flex-col gap-4">
            {/* Header with Title */}
            <div 
              className="w-full rounded-xl relative overflow-hidden p-4"
              style={{ 
                background: `linear-gradient(45deg, ${bgColor}, ${bgColor}E6)`
              }}
            >
            ...
            {/* Full Description */}
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-accent scrollbar-track-transparent pr-2">
              ...
              {('extras' in content && content.extras) && (
                <div className="mt-4">
                  <h4 className="text-accent font-semibold text-sm mb-2">Ce includem:</h4>
                  <ul className="space-y-1">
                    {(content.extras as string[]).map((extra: string, idx: number) => (
              ...
              <div className="flex justify-between items-center">
                <div className="flex flex-col">
                  ...
                  <div className="text-muted text-xs">
                    {content.duration} • {content.modules} module
                  </div>
                </div>
                
                <button
                  onClick={onCTAClick}
                  className="px-4 py-2 bg-accent rounded-full text-white text-sm font-semibold cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg"
                >
                  Abonare
                </button>
              </div>
