...
export function Pricing() {
  ...
  // Simplified card parallax for performance
  ...
  return (
    <Section
      ...
      ref={sectionRef}
    >
      <BackgroundEffects variant="dots" color="#ff4500" opacity={0.12} animated={false} />
...
function PricingCard({ plan, onCTAClick }: PricingCardProps) {
  const bgColor = plan.id === 'essential' ? '#ff4500' : plan.id === 'professional' ? '#6a00ff' : '#40CFFF';
        ...
        <div className="flex justify-between items-center mt-auto pt-3 border-t border-line">
          ...
          {plan.id === 'enterprise' && 'instagramUrl' in content ? (
            <a
              ...
              className="px-4 py-2 bg-accent rounded-full text-white text-[0.75em] font-semibold cursor-pointer transition-all duration-300 md:hover:scale-105 md:hover:shadow-[0_0_0_4px_rgba(255,69,0,0.2)] flex items-center gap-2"
            >
            ...
            <button
              onClick={onCTAClick}
              className="px-4 py-2 bg-accent rounded-full flex items-center justify-center text-white cursor-pointer transition-all duration-300 md:group-hover:scale-105 md:group-hover:shadow-[0_0_0_4px_rgba(255,69,0,0.2)]"
            >
